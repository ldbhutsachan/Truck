    ...
    public int setPaymentStatus(PayReq payReq) throws ParseException {
        ...
        try {
            if(payAmount == amount){
                ...
                SQL = "update PAYMENT_HIS set PAY_STATUS='O',PAY_AMOUNT=cast(replace(PAY_AMOUNT, ',', '') as unsigned)+'"+payAmount+"'  where BILLNO='" + payReq.getBillNo() + "' OR  INVOICE_NO='"+payReq.getInvoiceNo()+"'";

               // SQL = "update PAYMENT_HIS set PAY_STATUS='O',PAY_AMOUNT=cast(replace(PAY_AMOUNT, ',', '') as unsigned)+cast(replace("+payAmount+", ',', '') as unsigned) where BILLNO='" + payReq.getBillNo() + "' OR  INVOICE_NO='"+payReq.getInvoiceNo()+"'";
            }
            log.info("sql:"+SQL);
            return EBankJdbcTemplate.update(SQL);
        } catch (Exception e){
